...
;; 运行在实模式下,负责将ext.bin和kernel.bin
;; 读入0x7c00和51200物理地址处,然后跳转到0x7c00
;;
;; 为什么要把ext.bin放到0x7c00处运行? 忘了理由了
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
LABEL_BEGIN:
	...
	mov es, ax
	mov ss, ax
	mov sp, 0100h  ;比较随意的一个值，因为不使用栈

    ;把自己从7c00h(31K)处挪到9000h(36K)处
	;这是一个临时的地方，防止被ext.bin覆盖掉
    mov ecx, 512 ;计数
    mov ax, 0    
    mov es, ax
    mov ds, ax
...
;    jmp 900h:LLL  ; 继续从挪动后的代码里运行 
    ...
    ; 栈开始的地方，后面反复读扇区需要用到栈空间保存临时变量
    ; 注意分配的地方不要与现有代码覆盖，好好规划
	mov sp, 0f00h      
	...
	; 读1-4 扇区
	mov ax, BaseOfExt   ;07c0h ->ax
	mov es, ax
	mov bx, OffsetOfExt ; 0->bx
	mov ah, 02h ;读磁盘的子命令号
	mov al, 4 ;扇区数目
	mov ch, 0 ;磁道号低8位
...
.conti1:
	...
	;  5-17扇区
	mov ax, (50*1024)/16  ;放到从51200开始的缓冲区
	mov es, ax
	mov bx, 0
	mov ah, 02h ;读磁盘的子命令号
	mov al, 13 ;扇区数目
...
.ok2:
	...
	;开始运行ext.bin
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	jmp dword 0:BaseOfExt*16+OffsetOfExt ;  如果jmp dword BaseOfExt:OffsetOfExt 这样会导致cs不为0，后面不好弄
.fail:
	mov ax, 0B800h  ;显存的开始地址
	mov gs, ax
	mov al, 'E'      ;欲显示的字符
	mov ah, 0ch      ;颜色属性
	mov edi, (80*10+2)*2  ;第10行第2列
	mov [gs:edi], ax
	jmp $   ;死循环
