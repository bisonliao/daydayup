...
static int fNodeSwapOut(const unsigned char * key, size_t keysize, const unsigned char * val, size_t valsize)
{
	...
	if (buf->flags & BUFFER_FLAG_DIRTY)
	{
		...
		{
			panic("%s, %d:hd_add_request failed!", __FILE__, __LINE__);
		}
...
void fs_init()
{
	...
	uint32_t start = BUFFER_ORG;
	uint32_t size = BUFFER_END-BUFFER_ORG-1;
...
void buffer_init(uint32_t buffer_start, uint32_t buffer_size)
{
	...
	/*读取超级块*/
	{
		...
		{
			panic("%s %d: read super block failed!\n", __FILE__, __LINE__);
		}
		...
		{
			panic("%s, %d:hash_insert() failed!, iret=%d", __FILE__, __LINE__, iret);
		}
	}
	/*读取inode位图*/
	for (i = 0; i < NSECT_FOR_INODE_BMP; ++i)
	{
		...
		if (hash_insert(&hash,
					...
					(const unsigned char*)&g_tmp_buffer, sizeof(g_tmp_buffer)) )
		{
			panic("%s, %d:hash_insert() failed!", __FILE__, __LINE__);
		}
	}
	/*读取sect位图*/
...
TBuffer * buffer_lock(uint32_t abs_sect)
{
	...
	if (iret < 0)
	{
		panic("%s, %d:hash_find2() failed!", __FILE__, __LINE__);
	}
	if (iret == 1) /*木有找到 */
	{
		g_tmp_buffer.flags = 0;
		...
		if (iret = hash_insert(&hash,
					...
					(const unsigned char*)&g_tmp_buffer, sizeof(g_tmp_buffer)) )
		{
			panic("%s, %d:hash_insert() failed! abs_sect=%u, iret=%d", __FILE__, __LINE__, abs_sect, iret);
		}
		iret = hash_find2(&hash, (const unsigned char*)&abs_sect, sizeof(abs_sect), (void**)&bh);
		if (iret < 0)
		{
...
TBuffer * buffer_read(uint32_t abs_sect)
{
	...
	if (g_hd_sync_flag)
	{
		if (hd_read_sync(abs_sect, bh->data)	!= 0)
		{
			panic("%s %d:hd_read_sync() failed!\n", __FILE__, __LINE__);
		}
		bh->flags |= BUFFER_FLAG_UPTODATE;
	}
	else
	{
		if (iret = hd_add_request( bh, abs_sect, WIN_READ, g_current->pid) )
		{
			panic("%s, %d:hd_add_request failed! returned %d", __FILE__, __LINE__, iret);
		}
		g_current->status = PROC_STATUS_WAITING;
		schedule();	/*让其它进程开始运行，当前进程等待IO完成*/
	}
...
unsigned int alloc_block()
{
	...
	panic("%s %d: no free block available!", __FILE__, __LINE__);
	return 0;
}
...
void free_block(uint32_t nr)
{
	...
	{
		panic("%s %d: invalid block number:%u\n", 
				__FILE__,
				__LINE__,
				nr);
	}
