...
img_wd = 256
img_ht = 256
train_img_path = 'E:\\DeepLearning\\data\\CGAN\\%s/train' % (dataset)
val_img_path = 'E:\\DeepLearning\\data\\CGAN\\%s/val' % (dataset)

# 直接加载到内存，占用太多内存，我的机器配置不够
'''
...
class MyFileDataset(mx.gluon.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img_arr = mx.image.imresize(img_arr, img_wd * 2, img_ht)
        # Crop input and output images
        img_arr_in = mx.image.fixed_crop(img_arr, 0, 0, img_wd, img_ht)
        img_arr_out = mx.image.fixed_crop(img_arr, img_wd, 0, img_wd, img_ht)
        ...
        i = img_arr_out if self.is_reversed else img_arr_in
        o = img_arr_in if self.is_reversed else img_arr_out
        return i,o
    def __len__(self):
