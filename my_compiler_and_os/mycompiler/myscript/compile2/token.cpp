...
string CToken::ToString() const
{
	...
	{ 
		char buf[100];
		sprintf(buf, "%lld", m_yylval.int_val);
		rts = string("^") + buf;
	}
	...
	else if (m_token == MINUS) { rts = "MINUS";}
...
int CToken::GetOprntNumNeed() const
{
	...
	else if (m_token == MINUS) { return 1; }
	else { return -1;}
...
int CToken::GetPriorityIN() const
{
	...
	else if (m_token == ADD || m_token == SUB || m_token == MINUS)
	{
		return 2;
	}
