...
%token IF THEN ELSE ENDIF WHILE DO ENDWHILE INTEGER STRING FLOAT RETURN BEGIN_FLOW END_FLOW RUN FOR ENDFOR CONTINUE BREAK REPEAT UNTIL SWITCH ENDSWITCH CASE
...
statement	:ID '=' expr ';' 	{
		...
		|RUN '(' expr ')' ';' {
				...
				//为什么不用一个简单的GOTO语句来实现流程块之间的调用/跳转呢？
				//原因是被调用的流程的名字可能是一个复杂的表达式，而不仅仅是一个简单的常量字符串
				//也就是说目标流程名在编译期间不可知，所以要作为变量保存在运算栈中
				//能想到这点我太牛B了，哈哈。
