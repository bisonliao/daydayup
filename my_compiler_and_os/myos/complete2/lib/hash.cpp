...
#define NULL 0
extern "C" void memset(void * p, unsigned char v, size_t sz);
extern "C" void memcpy(void * dst, const void * src, size_t sz);
extern "C" int memcmp(const void * a, const void * b, size_t sz);
...
{
	unsigned int i;
	this->m_pHead->free_num = 0;
	this->m_pHead->used_num = 0;
	this->m_pHead->free_head = INVALID_NODE;
	this->m_pHead->used_head = INVALID_NODE;
	this->m_pHead->used_tail = INVALID_NODE;
...
int CHash::RemoveNodeFromFreeList(int node)
{
	...
	CHashNode * pstNodeNext = (CHashNode *)NULL, *pstNodePrev = (CHashNode *)NULL;
...
int CHash::GetFreeNode(int & node)
{
	...
	if (m_pHead->used_tail != INVALID_NODE)
	{
		...
		if (m_fNodeSwapOut)
		{
			...
			m_fNodeSwapOut(pstNode->acKey, pstNode->iKeySize,
					pstNode->acVal+m_iMaxKeySize, pstNode->iValSize);
		}
...
#else
	//挨个查找used链表，由上层应用决定是否能淘汰
	node = m_pHead->used_tail;
	if (m_fNodeSwapOut == NULL)
	{
		//printk(">>>%d\n", __LINE__);
		return -1;
	}
	...
	{
		CHashNode *pstNode = Node2Ptr(node);
		int iret = m_fNodeSwapOut((const unsigned char *)pstNode->acKey, 
				pstNode->iKeySize, (const unsigned char *)(pstNode->acVal+m_iMaxKeySize), pstNode->iValSize);
		if (iret < 0)
		{
			//printk(">>>%d\n", __LINE__);
			return -1;
...
void CHash::ScanUsedNode( void (*fScan)(const unsigned char * key, size_t keysize, const unsigned char * val, size_t valsize) )
{
	int node;
	
	node = m_pHead->used_head;
	if (fScan == NULL)
	{
		return;
	}
	...
	{
		CHashNode *pstNode = Node2Ptr(node);
		fScan((const unsigned char *)pstNode->acKey, 
				pstNode->iKeySize, (const unsigned char *)(pstNode->acVal+m_iMaxKeySize), pstNode->iValSize);
		node = pstNode->iNext;
	}
}
...
int CHash::RemoveNodeFromBucketList(int node)
{
	...
	//修改前后节点的指针
	if (pstNodeNext != (CHashNode *)NULL)
	{
		pstNodeNext->iPrevInBucket = iPrevInBucket;
	}
	if (pstNodePrev != (CHashNode *)NULL)
	{
...
CHash::CHash(size_t iBucketNum, size_t iMaxKeySize, size_t iMaxValSize, 
		...
		int (*fNodeSwapOut)(const unsigned char * key, size_t keysize, const unsigned char * val, size_t valsize) /*= NULL*/)
{
	m_pHead = (CHashHead *)NULL;
	m_iNodeNum = 0;
...
int CHash::MemAttach(void * ptr, size_t size, bool bCreat /*= false*/)
{
	...
	//printk("node number in hash is %u\n", m_iNodeNum);
	if (bCreat)
	{
		if (Initialize() < 0)
		{
...
int CHash::Find(int & node, int & bucketid, const unsigned char * keybuf, size_t keysize)
{
	...
	CHashNode *pstNode = (CHashNode *)NULL;
	int iNext = m_pHead->bucket[ bucketid ];
	while (iNext != INVALID_NODE)
	{
		pstNode = Node2Ptr(iNext);

		if ( m_fKeyEqual( (const unsigned char *)pstNode->acKey, pstNode->iKeySize,
					(const unsigned char *)keybuf, keysize))
		{
			node = iNext;
			return 0;
		}
		...
		const unsigned char * valbuf, size_t valsize)
{
	...
	if (iRet == 0) //已经存在,直接更新
	{
		...
		memcpy( (void *)(pstNode->acVal+m_iMaxKeySize), (const void *)valbuf, valsize);
		return 0;
	}
	//否则插入
	bucketid = m_fHash(keybuf, keysize) % m_iBucketNum;
	...
	pstNode->iValSize = valsize;
	memcpy( (void *)pstNode->acKey, (const void*)keybuf, keysize);
	memcpy( (void*)(pstNode->acVal + m_iMaxKeySize), (const void*)valbuf, valsize);
		...
		unsigned char * valbuf, size_t * pvalsize)
{
	...
	{
		memcpy( (void*)valbuf, (void*)(pstNode->acVal+m_iMaxKeySize), *pvalsize);
		return 2;
	}
...
int CHash::Find(const unsigned char * keybuf, size_t keysize, void * * ppVal)
{
	...
	CHashNode * pstNode = Node2Ptr(node);
	*ppVal = (void*)(pstNode->acVal+m_iMaxKeySize);
	return 0;
}
...
{
	if (sizeof(THashHead) < sizeof(CHash) || fNodeSwapOut == NULL)
	{
		return -1;
	}
...
{
	CHash * pHashClass = (CHash *)pstHead;
	return pHashClass->ScanUsedNode(fScan);
}
