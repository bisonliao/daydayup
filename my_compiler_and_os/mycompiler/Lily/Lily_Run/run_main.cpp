...
/*查找名字为name的变量,并把其地址作为一个字符串保存到ele中 */
bool FindVarAddrByName(const AnsiString& name,Variable& ele);
...
int ExecuteInstruct(const INSTRUCT & inst, char *errmsg)
{
	...
	if (strcmp(inst.inst_action, "CALL") == 0)
	{
		...
		for (int i = argc.getInteger(); i >= 1; i--)
		{
			...
			#ifdef _DEBUG
				if (argv[i].getType() == TYPE_STRING)
				{
					printf("[%s][%d][%s]=[%s]\n", 
							__FILE__,
							__LINE__,
							argv[i].getName().c_str(),
							argv[i].getString().c_str());
				}
				if (argv[i].getType() == TYPE_INTEGER)
				{
					printf("[%s][%d][%s]=[%d]\n", 
							__FILE__,
							__LINE__,
	...
	/*取变量的地址*/
	if (strcmp(inst.inst_action, "ADDR") == 0)
	{
		AnsiString name;
		Variable addr;

		name = inst.inst_operant1;
		if (FindVarAddrByName(name, addr) != TRUE)
		{
			sprintf(errmsg, "获取变量[%s]的地址失败!", name.c_str());
			return -1;
		}
		operation_stack.push(addr);
		return 0;
	}
...
bool FindVarAddrByName(const AnsiString& name,Variable& ele)
{
	...
	/*根据变量名字和当前模块的变量个数到数据栈中查找局部变量*/
	varnum_stack.peek(varnum);
	if (data_stack.FindVarAddrByNameFrmTop(name, varnum, ele) == TRUE)
	{
		return TRUE;
	}
	...
	if (1 == isGlobalID(name.c_str(), &index))
	{
		/*将变量的地址用16进制表示,存放在字符串里*/
		ele.setType(TYPE_STRING);
		ele.setName("");
		unsigned long addr = (unsigned long) &(global_var[index]);
		char tmp[100];
...
bool ModifyVarByName(const AnsiString &name, const Variable& ele)
{
	...
	/*到全局变量区查找*/
	int index;
	if (1 == isGlobalID(name.c_str(), &index) && ele.getType() == TYPE_STRING)
	{
