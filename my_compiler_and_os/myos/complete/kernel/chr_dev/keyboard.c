...
static  CycleBufHead *g_pstKBBufHead = NULL;
...
static inline void keyboard_init_buf()
{
	static int bDone = 0;
	if (!bDone)
	{
...
void keyboard_intr_handle()
{
	...
	busy_flag = busy_flag | TASK_BUSY_FLAG_KEYBOARD ;
	task_set_busy(busy_flag);
}
...
int keyboard_do_task()
{
	...
	for (i = 0; i < 10; ++i)
	{
		...
		{
			/* 没有扫描码需要处理了 */
			llTmp = TASK_BUSY_FLAG_KEYBOARD;
			llTmp = ~llTmp;
			task_is_busy(&busy_flag);
			busy_flag = busy_flag & llTmp;
			task_set_busy(busy_flag);
