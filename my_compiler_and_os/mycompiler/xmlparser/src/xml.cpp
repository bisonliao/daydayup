...
#ifdef _DEBUG
	#define SHOW_STEP {printf("运行到[%s][%d]\n", __FILE__, __LINE__);}
#endif
	...
	while (!stk.empty())
	{
		...
		else	/*不是叶子*/
		{
			...
			else /*遍历下一个孩子*/
			{
				...
				stk.push(stk_ele2);
				//把这个小孩压栈
				if (stk_ele.NodePtr->GetNodeChildList().empty())//小孩是叶子节点
				{
					xmlstring.concat("<");
					xmlstring.concat(stk_ele.NodePtr->GetNodeName());
					if (stk_ele.NodePtr->GetNodeProperty().length() != 0)
...
/*
*对字符串xxx进行转义处理:
*	<转为&lt,>转为&gt,&转为&amp,'转为&apos "转为&quot
*/
	...
	while (!stk.empty())
	{
		...
		else	/*不是叶子*/
		{
			...
			else /*遍历下一个孩子*/
			{
				...
				stk.push(stk_ele2);
				//把这个小孩压栈
				if (stk_ele.NodePtr->GetNodeChildList().empty())//该小孩是叶子节点
				{
					xmlstring.concat("<");
					xmlstring.concat(stk_ele.NodePtr->GetNodeName());
					if (stk_ele.NodePtr->GetNodeProperty().length() == 0)
