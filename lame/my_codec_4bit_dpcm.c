...
int DM_decode_frame(int fd, uint32_t *sample_rate, uint8_t *channel_num, short pcm_l[], short pcm_r[])
{
	frame_head_t head;
		...
		if (data_len < 0 || data_len != (head.sample_rate/2*head.channel_num) )
		{
			return -1;
		}
		...
		for (j = 0; j < head.channel_num; ++j)
		{
			...
			for (i = 0; i < head.sample_rate/2; ++i)
			{
				unsigned char code = buf[offset++];

				int delta = (code & 0x70) >> 4;
				int neg = (code & 0x80) >>7;
				if (neg) { delta = -delta;}
				pcm[2*i] = delta*head.step_size[j] + prev;
				prev = pcm[2*i];
...
int DM_get_frame_size(uint32_t sample_rate, uint8_t channel_num)
{
	return sizeof(frame_head_t) + (sample_rate/2) * channel_num;
}
				...
				short pcm_l[], short pcm_r[], unsigned char * frame_buffer)
{
	...
	pcm[0] = pcm_l;
	pcm[1] = pcm_r;
	uint8_t * codes = (uint8_t*)(frame_buffer+sizeof(frame_head_t));
	...
	for (i = 0; i < 2; ++i) // two channels
	{
		...
		for (j = 0; j < sample_rate; j +=2)  // encode one channel samples
		{

			// encode 2 sample once

			uint8_t * code = codes+offset;
			offset++;
			int neg1 = 0, neg2 = 0;
			...
			int delta2 = (pcm[i][j+1]-prev) / head->step_size[i];


			if (delta2 < 0) {neg2 = 1;delta2 = -delta2;}
			if (delta2 > 7) { delta2 = 7; overflow++;}

			prev = prev + delta2 *pow(-1, neg2)* head->step_size[i];


			*code = ((uint8_t)delta1 << 4) | (uint8_t)delta2;
			if (neg1) { *code = *code | 0x80;}
			if (neg2) { *code = *code | 0x08;}
		}
